...
func make_config(t *testing.T, n int, unreliable bool, funcName string) *config {
	ncpu_once.Do(func() {
		if runtime.NumCPU() < 2 {
...
//
func (cfg *config) start1(i int, funcName string) {
	cfg.crash1(i)
	...
	rf := Make(ends, i, cfg.saved[i], applyCh, funcName)
