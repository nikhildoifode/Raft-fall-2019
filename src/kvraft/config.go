...
func (cfg *config) makeClient(to []int, funcName string) *Clerk {
	cfg.mu.Lock()
	defer cfg.mu.Unlock()
	...
	ck := MakeClerk(random_handles(ends), funcName)
...
func (cfg *config) StartServer(i int, funcName string) {
	cfg.mu.Lock()
	...
	cfg.kvservers[i] = StartKVServer(ends, i, cfg.saved[i], cfg.maxraftstate, funcName)
...
func make_config(t *testing.T, n int, unreliable bool, maxraftstate int, funcName string) *config {
	ncpu_once.Do(func() {
		if runtime.NumCPU() < 2 {
